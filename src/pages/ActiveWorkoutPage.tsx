...
export default function ActiveWorkoutPage() {
  ...
  // Experienced mode: track per-exercise completion status
  ...
  useEffect(() => {
    if (settings.experiencedMode) {
      currentRowRef.current?.scrollIntoView({ behavior: 'smooth', block: 'center' })
    } else {
      exerciseContentRef.current?.scrollTo(0, 0)
    }
  }, [currentIndex, settings.experiencedMode])
  ...
  return (
    ...
    >
      ...
      {settings.experiencedMode ? (
        /* ── Experienced Mode: Compact Checklist ── */
        <div ref={exerciseContentRef} className="flex-1 overflow-y-auto hide-scrollbar px-3 py-2 min-h-0">
          {(() => {
            let lastPhase = ''
            return flatExercises.map((item, idx) => {
              const isCurrent = idx === currentIndex
              const isDone = completedIndices.has(idx)
              const isSkipped = skippedIndices.has(idx)
              const isPast = isDone || isSkipped
              const showPhaseHeader = item.phase !== lastPhase
                ...
                <div key={idx}>
                  {showPhaseHeader && (
                    <div className="flex items-center gap-2 mt-3 mb-1.5 first:mt-0">
                      <div className={cn(
                        'w-1.5 h-1.5',
                        item.phase === 'Warm-Up' ? 'bg-star-gold' :
                          item.phase === 'Cool-Down' ? 'bg-khaki' : 'bg-primary-500'
                      )} />
                      <span className="text-[10px] font-heading font-bold tracking-widest text-text-muted uppercase">
                        {item.phase}
                      </span>
                  ...
                  <motion.div
                    ...
                    layout
                    className={cn(
                      'flex items-center gap-2.5 px-3 py-2.5 mb-1 border transition-all',
                      isCurrent
                        ? 'bg-primary-900/40 border-primary-500'
                        : isPast
                          ? 'bg-surface-0 border-surface-2 opacity-50'
                          : 'bg-surface-1 border-surface-3'
                    )}
                    style={isCurrent ? { boxShadow: '0 0 12px color-mix(in srgb, var(--color-primary-500) 15%, transparent)' } : {}}
                  >
                    {/* Status indicator */}
                    <div className={cn(
                      'w-6 h-6 border-2 flex items-center justify-center shrink-0 transition-all',
                      isDone ? 'border-primary-500 bg-primary-500' :
                        isSkipped ? 'border-text-ghost bg-surface-2' :
                          isCurrent ? 'border-primary-500' : 'border-surface-3'
                    )}>
                      {isDone && (
                        <motion.svg
                      ...
                      {isSkipped && (
                        <svg className="w-3 h-3 text-text-ghost" fill="none" stroke="currentColor" strokeWidth={2.5} viewBox="0 0 24 24">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={cn(
                        'text-xs font-heading font-bold tracking-wider uppercase truncate',
                        isCurrent ? 'text-primary-500' :
                          isPast ? 'text-text-ghost' : 'text-text-primary'
                      )}>
                        {item.exercise.exercise.name}
                      </p>
                      {item.totalRounds > 1 && (
                        <span className="text-[9px] font-mono text-text-ghost">
                          {settings.exerciseGrouping === 'grouped' ? 'SET' : 'RND'} {item.roundNum}/{item.totalRounds}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="shrink-0 text-right">
                      {item.exercise.durationSeconds ? (
                        <span className={cn(
                          'font-mono text-sm font-bold',
                          isCurrent ? 'text-primary-500' : 'text-text-muted'
                        )}>
                          {isCurrent && timer.isRunning ? timer.secondsLeft : item.exercise.durationSeconds}S
                        </span>
                      ) : (
                        <span className={cn(
                          'font-mono text-sm font-bold',
                          ...
                          {item.exercise.reps}{item.exercise.perSide ? 'ea' : ''}
                        </span>
                      )}
                    </div>
                  </motion.div>
        ...
        /* ── Standard Mode: Detailed Single Exercise ── */
