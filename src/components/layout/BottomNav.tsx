...
const baseTabs = [
  ...
  { path: '/plan', label: 'PLAN', icon: 'M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z', conditional: false },
  ...
  { path: '/coach', label: 'COACH', icon: 'M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z', conditional: true },
...
export default function BottomNav() {
  ...
  const tabs = useMemo(() => {
    // Filter out conditional tabs based on settings
    return baseTabs.filter(tab => {
      if (tab.conditional) {
        // COACH tab only shows if AI is enabled and API key is set
        return settings.enableAICoach && settings.groqApiKey
      }
      return true
    })
  }, [settings.enableAICoach, settings.groqApiKey])
